...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
retry:
	dentry = d_alloc(base, found_sus_path ? &susfs_fake_qstr_name : name);
#else
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (SUSFS_IS_INODE_SUS_PATH(d_backing_inode(dentry))) {
		dput(dentry);
		return -ENOENT;
	}
#endif
...
retry:
	...
	for (;;) {
		if (!dentry) {
			const struct qstr *lookup_name = &nd->last;

#ifdef CONFIG_KSU_SUSFS_SUS_PATH
			if (found_sus_path)
				lookup_name = &susfs_fake_qstr_name;
#endif
			dentry = d_alloc_parallel(dir, lookup_name, &wq);
			if (IS_ERR(dentry))
		   ...
		   struct file *file, const struct open_flags *op)
{
	...
	if (file->f_mode & FMODE_OPENED) {
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
		if (SUSFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(file_inode(file)) &&
		    susfs_open_redirect_should_redirect(file_inode(file))) {
			error = -EOPENREDIRECT;
			goto out;
		}
#endif
...
finish_open_created:
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	inode = d_inode(nd->path.dentry);
	if (SUSFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(inode) &&
	    susfs_open_redirect_should_redirect(inode)) {
		error = -EOPENREDIRECT;
		goto out;
	}
#endif
...
redirect_open:
	...
	if (IS_ERR(file)) {
		...
		if (fake_filename && !IS_ERR(fake_filename)) {
			restore_nameidata();
			set_nameidata(nd, old_dfd, original_filename);
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
		if (error == -EOPENREDIRECT) {
			struct inode *inode = file->f_mode & FMODE_OPENED ?
				file_inode(file) : d_inode(nd->path.dentry);

			fake_filename =
				susfs_open_redirect_spoof_do_filp_open(inode);
		}
#endif
		terminate_walk(nd);
	}
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	if (error == -EOPENREDIRECT && fake_filename &&
	    !IS_ERR(fake_filename)) {
		fput(file);
		restore_nameidata();
	...
	if (error == -EOPENREDIRECT) {
		error = IS_ERR(fake_filename) ? PTR_ERR(fake_filename) : -ENOENT;
		fake_filename = NULL;
	}
#endif
	if (likely(!error)) {
	...
	if (unlikely(!(inode->i_opflags & IOP_DEFAULT_READLINK))) {
		if (unlikely(inode->i_op->readlink)) {
			...
			if (SUSFS_IS_INODE_OPEN_REDIRECT(inode)) {
				res = susfs_spoof_vfs_readlink(inode, buffer,
							       buflen);
				if (res >= 0)
					return res;
			}
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	if (SUSFS_IS_INODE_OPEN_REDIRECT(inode)) {
		res = susfs_spoof_vfs_readlink(inode, buffer, buflen);
		if (res >= 0) {
