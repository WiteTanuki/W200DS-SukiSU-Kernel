...
/*zte add for read chip probe state*/
unsigned int zte_aw87xxx_dev_cnt = 0;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	AW_DEV_LOGI(aw87xxx->dev, "dev_index = %d set profile [%s]", aw87xxx->dev_index, profile_name);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	AW_DEV_LOGI(aw87xxx->dev, "dev_index = %d current profile:[%s]", aw87xxx->dev_index,
		aw87xxx->current_profile);
...
static int aw87xxx_mode_switch_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	char *profile_name = NULL;
	...
	aw87xxx->mode_profile = profile_name;
						...
						void *codec)
{
	...
	snprintf(kctl_name[3], AW_NAME_BUF_MAX, "aw87xxx_mode_switch_%d",
			aw87xxx->dev_index);

	aw87xxx_kcontrol[3].name = kctl_name[3];
	aw87xxx_kcontrol[3].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw87xxx_kcontrol[3].info = aw87xxx_profile_switch_info;
	aw87xxx_kcontrol[3].get = aw87xxx_profile_switch_get;
	aw87xxx_kcontrol[3].put = aw87xxx_mode_switch_put;
...
void aw87xxx_get_mode_profile(int dev_index, char **profile_name)
{
	struct list_head *pos = NULL;
	struct aw87xxx *aw87xxx = NULL;
	...
	list_for_each(pos, &g_aw87xxx_list) {
		aw87xxx = list_entry(pos, struct aw87xxx, list);
		if (aw87xxx->dev_index == dev_index) {
			*profile_name = aw87xxx->mode_profile;
		}
	}
}
EXPORT_SYMBOL(aw87xxx_get_mode_profile);
...
/*zte add for chip probe state, init value as 0,
every chip probe success increase value 1*/
bool zte_get_aw873xx_chip_state(void)
{
	return zte_aw87xxx_dev_cnt >= 1;
}
EXPORT_SYMBOL(zte_get_aw873xx_chip_state);
				...
				const struct i2c_device_id *id)
{
	...
	aw87xxx_dev_hw_reset(&aw87xxx->aw_dev);
