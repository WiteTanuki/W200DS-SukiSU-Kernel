...
static int sqc_apdo_enable = SQC_PPS_DEFAULT;
static int sqc_pd_voltage = 5000; /*5000mv*/
static int sqc_pps_currnet = 0;
...
static int pd_status_init(void)
{
	...
	sqc_apdo_enable = SQC_PPS_DEFAULT;
	sqc_pps_currnet = 0;
...
static int pd_set_sqc_cap(int type, int mV, int mA)
{
	...
	if  (type == SQC_PDOS_TYPE_APDO) {
		if (sqc_apdo_enable != SQC_PPS_ENABLE) {
			sqc_apdo_enable = SQC_PPS_ENABLE;

			pr_info("[SQC-HW]: [%s] enable pps\n", __func__);

			ret = sprd_enable_pps(info, true); /*will request PPS 5V*/
			if (ret)
				dev_err(info->dev, "[SQC-HW]%s sprd_enable_pps failed enable, ret:%d\n",
					__func__, ret);

			msleep(200); /*avoid rapid repeated requests, this may cause hardreset or packet loss*/
		...
		if (sqc_apdo_enable != SQC_PPS_DISABLE) {
			sqc_apdo_enable = SQC_PPS_DISABLE;

			pr_info("[SQC-HW]: [%s] disable pps\n", __func__);

			ret = sprd_enable_pps(info, false); /*will request PD 5V*/
			if (ret)
		...
		} else if (sqc_pd_voltage != mV) {
			/*pps switching to pd, most of them request PD 5V*/
			/*"else" is avoid rapid repeated requests 5V, this may cause hardreset*/
			sqc_pd_voltage = mV;
			pr_info("[SQC-HW]: [%s] adjust pd voltage %d\n", __func__, mV);
