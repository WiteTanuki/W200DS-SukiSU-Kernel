...
#define SC27XX_FGU_BOOT_CAPACITY_THRESHOLD			50
...
{
	int ret;
	int start_cap = 0, start_clbcnt = 0;

	start_cap = data->init_cap;
	start_clbcnt = data->init_clbcnt;
	...
	dev_info(data->dev, "%s Charged/Discharged from %d to %d, delta_clbcnt:%d, mah:%d\n", __func__, start_cap, cap,
		(data->init_clbcnt - start_clbcnt), sc27xx_fgu_clbcnt2mah(data, data->init_clbcnt - start_clbcnt));
}
...
{
	int pocv_uv, ret, pocv_cap, ocv_mv, ocv_cap;
	...
	if (ret) {
		...
		ocv_cap = sc27xx_fgu_ocv2cap(data->cap_table, data->table_len, ocv_mv * 1000);
	}
	...
	if (!is_first_poweron) {
		...
		dev_info(data->dev, "not first_poweron, pocv_cap = %d, ocv_cap = %d, save_cap = %d\n", pocv_cap, ocv_cap, *cap);

		if (abs(ocv_cap - *cap) >= SC27XX_FGU_BOOT_CAPACITY_THRESHOLD) {
		     *cap = ocv_cap;
		}
		...
		pr_err("lsc: normal_cap, *cap = 0x%x\n", *cap);
				...
				union power_supply_propval *pval)
{
	...
	pr_info("%s:line%d: data->batt_id_index=%d, psp=%d\n", __func__, __LINE__, data->batt_id_index, psp);
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	dev_err(dev, "start to register zte fgu power supply");
	fgu_cfg.drv_data = data;
	fgu_cfg.of_node = np;
