...
static int sgm41510_charger_hw_init(struct sgm41510_charger_info *info)
{
	...
	ret = sgm41510_charger_set_termina_vol(info, info->vol_max_mv);
	if (ret) {
	...
	ret = sgm41510_charger_set_limit_current(info, info->cur.unknown_cur, false);
	if (ret)
...
static int sgm41510_charger_feed_watchdog(struct sgm41510_charger_info *info)
{
	...
	ret = sgm41510_charger_set_limit_current(info, 0, true);
	if (ret)
					    ...
					    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		ret = sgm41510_charger_set_limit_current(info, val->intval, false);
		if (ret < 0)
...
static const struct regulator_desc sgm41510_charger_vbus_desc = {
	.name = "sgm41510_otg_vbus",
	.of_match = "sgm41510_otg_vbus",
