...
static enum power_supply_usb_type sprd_vchg_get_bc1p2_type(struct sprd_vchg_info *info)
{
	...
	/*add by zte, force report float type when pd hard reset or typec connect*/
	if (!IS_ERR_OR_NULL(info->typec_extcon)
			&& extcon_get_state(info->typec_extcon, EXTCON_USB)
			&& (usb_type == POWER_SUPPLY_USB_TYPE_UNKNOWN)) {
	...
	if (!IS_ERR_OR_NULL(info->typec_extcon)
			&& (usb_type == POWER_SUPPLY_USB_TYPE_UNKNOWN)
			&& info->limit) {
		dev_err(info->dev, "%s: typec_online force report float limit=%d\n", SPRD_VCHG_TAG, info->limit);
		usb_type = POWER_SUPPLY_USB_TYPE_NON_DCP;
	}
...
static void sprd_vchg_detect_status(struct sprd_vchg_info *info)
{
	...
	if (info->use_typec_extcon) {
		 ...
		 * the charge current.
		 */
		dev_info(info->dev, "%s: chg_state=%d\n", __func__, info->usb_phy->chg_state);
		if (info->usb_phy->chg_state != USB_CHARGER_PRESENT)
			return;
