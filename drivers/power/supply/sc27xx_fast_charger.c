...
#ifdef	CONFIG_FAST_CHARGE_FOR_CHENYANG_ADAPT
#define SC2730_IB_TRIM_OFFSET			0x32
#elif defined CONFIG_IB_TRIM_OFFSET_VAL
#define SC2730_IB_TRIM_OFFSET			CONFIG_IB_TRIM_OFFSET_VAL
#else
...
#ifdef	CONFIG_IB_TRIM_OFFSET_VAL
#define UMP9620_IB_TRIM_OFFSET			CONFIG_IB_TRIM_OFFSET_VAL
#else
...
#define FCHG_DROP_TIME_MS		1500

#define FCHG_DEBUG_NUM			0xa55a
...
struct sc27xx_fchg_info {
	...
	s64 start_sfcp_time_ms;
	s64 plug_out_time_ms;
	u32 abnormal_cnt;
	int ib_trim_offset_val_debug;
};
...
static int sc27xx_fchg_internal_cur_calibration(struct sc27xx_fchg_info *info)
{
	...
	if (info->ib_trim_offset_val_debug == FCHG_DEBUG_NUM) {
		calib_current += pdata->ib_trim_offset;
		pr_info("normal: current source improve is:%d\n", pdata->ib_trim_offset);
	} else {
		calib_current += info->ib_trim_offset_val_debug;
		pr_info("debug: current source improve is:%d\n", info->ib_trim_offset_val_debug);
	}

	if (calib_current < 0) {
		calib_current = 0;
	} else if (calib_current > ANA_REG_IB_TRIM_MAX) {
		dev_err(info->dev, "calib_current = %d. set to 0x7F\n", calib_current);
		calib_current = ANA_REG_IB_TRIM_MAX;
	}
...
static int sc27xx_fchg_get_detect_status(struct sc27xx_fchg_info *info)
{
	 ...
	 */
	if (info->usb_phy->chg_type != DCP_TYPE && info->usb_phy->chg_type != NON_DCP_TYPE)
		return POWER_SUPPLY_USB_TYPE_UNKNOWN;

	if (info->abnormal_cnt > 1) {
		dev_info(info->dev, "[%s]abnormal_cnt = %d", __func__, info->abnormal_cnt);
		info->abnormal_cnt = 0;
		return POWER_SUPPLY_USB_TYPE_UNKNOWN;
	}
	...
	info->start_sfcp_time_ms = ktime_to_ms(ktime_get_boottime());
	dev_info(info->dev, "[%s]start_sfcp_time_ms = %lld\n", __func__, info->start_sfcp_time_ms);
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (val->intval == true) {
			if ((cur_time - info->plug_out_time_ms) >= 20000 &&
								info->abnormal_cnt)
				info->abnormal_cnt = 0;
			...
			info->plug_out_time_ms = cur_time;
			if ((cur_time - info->start_sfcp_time_ms) > 0 &&
				(cur_time - info->start_sfcp_time_ms) <= FCHG_DROP_TIME_MS)
					info->abnormal_cnt++;
			else
				info->abnormal_cnt = 0;
			complete(&info->completion);
		...
		dev_info(info->dev, "[%s]cur_time = %lld, start_sfcp_time_ms = %lld, "
			"plug_out_time_ms = %lld, delta = %lld, abnormal_cnt = %d\n",
			__func__, cur_time, info->start_sfcp_time_ms, info->plug_out_time_ms,
			(cur_time - info->start_sfcp_time_ms), info->abnormal_cnt);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
...
static int sfcp_get_prop_by_name(const char *name, enum power_supply_property psp, int *data)
{
	...
	rc = power_supply_get_property(psy,
				psp, &val);
	if (rc < 0) {
		pr_info("Failed to get %s property:%d rc=%d\n", name, psp, rc);
		return rc;
	}
...
static int sfcp_get_charger_type(int *chg_type)
{
	...
	if (!info->charger_online && !info->usb_phy->chg_type) {
		sfcp_get_prop_by_name("usb", POWER_SUPPLY_PROP_PRESENT, &usb_present);
		if (usb_present) {
...
out_loop:
	pr_info("[SQC-HW]: [%s] charger_online: %d, sprd_type: %d, chg_type: %d, usb_present: %d\n",
		__func__, info->charger_online, info->usb_phy->chg_type, *chg_type, usb_present);
...
int ib_trim_offset_val_set(const char *val, const void *arg)
{
	struct sc27xx_fchg_info *info = (struct sc27xx_fchg_info *) arg;
	int ib_trim_offset_val = 0;

	if (!info) {
		pr_info("sc27xx_fchg_info is null\n");
		return -EINVAL;
	}

	sscanf(val, "%d", &ib_trim_offset_val);
	info->ib_trim_offset_val_debug = ib_trim_offset_val;

	pr_info("set info->ib_trim_offset_val_debug = %d\n", ib_trim_offset_val);

	return 0;
}

int ib_trim_offset_val_get(char *val, const void *arg)
{
	struct sc27xx_fchg_info *info = (struct sc27xx_fchg_info *) arg;
	...
	pr_info("get info->ib_trim_offset_val_debug = %d\n", info->ib_trim_offset_val_debug);

	return snprintf(val, PAGE_SIZE, "%d", info->ib_trim_offset_val_debug);
}

struct zte_misc_ops ib_trim_offset_val_node = {
	.node_name = "ib_trim_offset_val",
	.set = ib_trim_offset_val_set,
	.get = ib_trim_offset_val_get,
	.free = NULL,
	.arg = NULL,
};
...
#endif
	info->ib_trim_offset_val_debug = FCHG_DEBUG_NUM;
	zte_misc_register_callback(&ib_trim_offset_val_node, info);
	return 0;
}
