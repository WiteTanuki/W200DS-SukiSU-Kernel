...
#define ZTE_BATTERY_RESERVED_SOC    100
#ifdef ZTE_FEATURE_SOC_RESERVED
#define CUTOFF_VOLTAGE_UV_RESERVED 3600000
/* Set uvlo shutdown voltage to 3300mV. it will added 120mV by default */
#define UVLO_CUTOFF_VOLTAGE_UV_RESERVED (CUTOFF_VOLTAGE_UV_RESERVED - 300000)
/* Trigger uvlo_work when voltage less then 3570mV */
#define ZTE_CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD	(CUTOFF_VOLTAGE_UV_RESERVED - 30000)
#define ZTE_CM_LOW_TEMP_SHUTDOWN_VALTAGE		ZTE_CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD
#define ZTE_CM_LOW_CAP_SHUTDOWN_VOLTAGE_THRESHOLD	ZTE_CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD
static int zte_conv_ui_cap = 500;
static int zte_soc_reserved_flag = 1;
#endif
...
#ifdef ZTE_FEATURE_SOC_RESERVED
static void zte_reserved_soc_conversion(int start_reserved_soc, int max_soc, int input_soc, int *output_soc);
#endif
...
static bool cm_zte_is_support_fchg_detect(struct charger_manager *cm)
{
	struct zte_power_supply *psy = NULL;

	/* slave chg register faild not support fchg */
	if (cm->desc->zte_psy_hardware2) {
		psy = zte_power_supply_get_by_name(cm->desc->zte_psy_hardware2);
		if (!psy) {
			dev_err(cm->dev, "get zte_psy_hardware2 psy failed, disable fchg\n");
			return false;
		}
		return true;
	}
	return true; /* no influence single buck support fchg */
}
...
{
	if (!cm_zte_is_support_fchg_detect(cm))
		return;
...
static void cm_get_uisoc(struct charger_manager *cm, int *uisoc)
{
	...
	if (cm->desc->cap_debug != -1)
		*uisoc = cm->desc->cap_debug;
		...
		if (zte_soc_reserved_flag == 1) {
			cap_new  = zte_conv_ui_cap;
		} else {
			cap_new  = cm->desc->cap;
		}
		cap_new = DIV_ROUND_CLOSEST(cap_new * 101, cm->desc->fullbatt_advance_level);
		dev_info(cm->dev, "zte_soc_reserved_flag:%d, capacity orig: %d, zte_conv_ui_cap:%d, new: %d\n",
			zte_soc_reserved_flag, cm->desc->cap, zte_conv_ui_cap, cap_new);
		...
		if (cap_new <= 0) {
			...
			/* TODO: need confirm the cutoff voltage for reserved soc condition */
			if (zte_soc_reserved_flag == 1) {
				if (batt_uV >= CUTOFF_VOLTAGE_UV_RESERVED) {
					cap_new = 1;
					dev_info(cm->dev, "## zte_soc_reserved force report soc 1, vbat %d\n", batt_uV);
				}
			} else {
...
static int cm_get_capacity_level_critical(struct charger_manager *cm)
{
	...
	int shutdown_voltage_threshold = CM_LOW_CAP_SHUTDOWN_VOLTAGE_THRESHOLD;

	if (zte_soc_reserved_flag == 1) {
		shutdown_voltage_threshold  = ZTE_CM_LOW_CAP_SHUTDOWN_VOLTAGE_THRESHOLD;
	}
	...
	if (is_charging(cm) && batt_uA > 0 &&
		batt_uv > shutdown_voltage_threshold) {
	...
	if (batt_uv >= shutdown_voltage_threshold) {
		dev_info(cm->dev, "##more than cutoff voltage, force level low, vbat:%d\n", shutdown_voltage_threshold);
		level = POWER_SUPPLY_CAPACITY_LEVEL_LOW;
	}
...
static int cm_get_capacity_level(struct charger_manager *cm)
{
	...
	if (zte_soc_reserved_flag == 1) {
		uisoc = DIV_ROUND_CLOSEST(zte_conv_ui_cap, 10);
	} else {
...
#ifdef ZTE_CHARGER_DETAIL_CAPACITY
#ifdef ZTE_FEATURE_SOC_RESERVED
	int zte_conv_raw_cap = 0;
#endif
#endif

	if (psp != POWER_SUPPLY_PROP_SET_SHIP_MODE && !is_ext_pwr_online(cm)
		&& (psp != POWER_SUPPLY_PROP_CAPACITY_RAW) && (psp != POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED)) {
...
#ifdef ZTE_FEATURE_SOC_RESERVED
			if (zte_soc_reserved_flag == 1) {
				zte_reserved_soc_conversion(ZTE_BATTERY_RESERVED_SOC*10, CM_CAP_FULL_PERCENT*10, cm->desc->cap*10, &zte_conv_raw_cap);
				cm->desc->raw_soc = DIV_ROUND_CLOSEST(zte_conv_raw_cap * 100, (cm->desc->fullbatt_advance_level / 10));
			} else {
...
static void cm_uvlo_check_work(struct work_struct *work)
{
	...
	int batt_uV, ret;
	int uvlo_threshold = CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD;
	int uvlo_calc_cap = cm->desc->cap;

	if (zte_soc_reserved_flag == 1) {
		cm->desc->shutdown_voltage = UVLO_CUTOFF_VOLTAGE_UV_RESERVED;
		uvlo_threshold  = ZTE_CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD;
		uvlo_calc_cap = zte_conv_ui_cap;
	}
	...
	if (cm->desc->uvlo_trigger_cnt >= CM_UVLO_CALIBRATION_CNT_THRESHOLD) {
		if (DIV_ROUND_CLOSEST(uvlo_calc_cap, 10) <= 1) {
...
static void cm_raw_soc_works(struct work_struct *work)
{
	...
	if (zte_soc_reserved_flag == 1) {
		zte_reserved_soc_conversion(ZTE_BATTERY_RESERVED_SOC*10, CM_CAP_FULL_PERCENT*10, cm->desc->cap*10, &zte_conv_raw_cap);
		raw_cap = DIV_ROUND_CLOSEST(zte_conv_raw_cap * 100, (cm->desc->fullbatt_advance_level / 10));
	} else {
	...
	if (bat_uA > 1000000) {
		...
		if (cm->desc->raw_soc < raw_cap) {
			schedule_delay = schedule_delay - delta_time/2;
		}

		if (cm->desc->raw_soc > raw_cap) {
			schedule_delay = schedule_delay + 2*delta_time;
		}
	} else {
		if (abs(cm->desc->raw_soc - raw_cap) <= 100) {
			...
			schedule_delay = CM_CAP_SOC_UPDATE_MIN_TIME*40;
...
static void zte_reserved_soc_conversion(int start_reserved_soc, int max_soc, int input_soc, int *output_soc)
{
	...
	/*int ret = 0;*/
	if ((output_soc == NULL) || (start_reserved_soc >= max_soc) || (start_reserved_soc < 0)) {
		pr_err("%s, Invalid parameters, start_reserved_soc or output_soc is valid\n", __func__);
		return;
	}

	/* Calculated by using the proportion method.
		output_soc:max_soc = (input_soc-start_reserved_soc):(max_soc-start_reserved_soc) */
	if (input_soc <= start_reserved_soc) {
		*output_soc = 0;
	} else if (input_soc >= max_soc) {
		*output_soc = max_soc;
	} else {
		*output_soc = max_soc*(input_soc - start_reserved_soc)/(max_soc - start_reserved_soc);
	}

	pr_info("%s,start_reserved_soc:%d,input_soc:%d, output_soc:%d\n",
		__func__, start_reserved_soc, input_soc, *output_soc);

	if (*output_soc > max_soc)
		*output_soc = max_soc;
	else if (*output_soc < 0)
		*output_soc = 0;
}
#endif
...
static void cm_batt_works(struct work_struct *work)
{
...
#endif
#ifdef ZTE_FEATURE_SOC_RESERVED
	static int zte_soc_reserved_en = 1;
	int zte_conversion_cap = 0;
#endif
	...
	if (cur_temp <= CM_LOW_TEMP_REGION &&
	    batt_uV <= ZTE_CM_LOW_TEMP_SHUTDOWN_VALTAGE) {
	...
	*/
...
#ifdef ZTE_FEATURE_SOC_RESERVED
	if (batt_uV < ZTE_CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD && ibat_avg < 0) {
		if (++uvlo_check_cnt > 2) {
			cm->desc->cap_one_time = CM_CAP_ONE_TIME_20S;
			work_cycle = CM_CAP_CYCLE_TRACK_TIME_10S;
		}
	...
	if (batt_uV < ZTE_CM_UVLO_CALIBRATION_VOLTAGE_THRESHOLD) {
...
#ifdef ZTE_FEATURE_SOC_RESERVED
	if (zte_soc_reserved_flag == 1) {
		zte_reserved_soc_conversion(ZTE_BATTERY_RESERVED_SOC, CM_CAP_FULL_PERCENT, fuel_cap, &zte_conversion_cap);

		dev_info(cm->dev, "fuel_cap = %d round to %d, cm->desc->cap = %d round to %d,"
			"zte_conversion_cap = %d round to %d, zte_conv_ui_cap = %d round to %d\n",
			fuel_cap, DIV_ROUND_CLOSEST(fuel_cap * 101, cm->desc->fullbatt_advance_level),
			cm->desc->cap, DIV_ROUND_CLOSEST(cm->desc->cap * 101, cm->desc->fullbatt_advance_level),
			zte_conversion_cap, DIV_ROUND_CLOSEST(zte_conversion_cap * 101, cm->desc->fullbatt_advance_level),
			zte_conv_ui_cap, DIV_ROUND_CLOSEST(zte_conv_ui_cap * 101, cm->desc->fullbatt_advance_level));

		if (zte_conversion_cap != zte_conv_ui_cap) {
			if (DIV_ROUND_CLOSEST(zte_conversion_cap * 101, cm->desc->fullbatt_advance_level)
				!= DIV_ROUND_CLOSEST(zte_conv_ui_cap * 101, cm->desc->fullbatt_advance_level)) {
				dev_info(cm->dev, "Percent Changed! zte_conversion_cap = %d, ROUND:%d, zte_conv_ui_cap = %d, ROUND:%d\n",
				zte_conversion_cap, DIV_ROUND_CLOSEST(zte_conversion_cap * 101, cm->desc->fullbatt_advance_level),
				zte_conv_ui_cap, DIV_ROUND_CLOSEST(zte_conv_ui_cap * 101, cm->desc->fullbatt_advance_level));
				zte_conv_ui_cap = zte_conversion_cap;
		...
		if (zte_soc_reserved_en != zte_soc_reserved_flag) {
				dev_info(cm->dev, "SOC MODE Changed! zte_soc_reserved_en = %d\n", zte_soc_reserved_en);
				zte_conv_ui_cap = zte_conversion_cap;
				cm->desc->cap = fuel_cap;
				cm->desc->update_capacity_time = cur_time.tv_sec;
				power_supply_changed(cm->charger_psy);
				zte_soc_reserved_en = 1;
		}
	} else {
		dev_info(cm->dev, "zte_conversion_cap = %d, new_uisoc = %d, old_uisoc = %d\n",
			zte_conversion_cap, fuel_cap, cm->desc->cap);
	...
	if ((cm->desc->charger_status == POWER_SUPPLY_STATUS_CHARGING)
		...
		schedule_delay = CM_CAP_CYCLE_TRACK_TIME*1000; /* Default schedule timer is 10s */
	}
...
extern int sc27xx_typec_set_rp_rd(int cc);

int cc_connect_set(const char *val, const void *arg)
{
	struct charger_manager *cm = (struct charger_manager *) arg;
	int connect;
	int num = 0;
	...
	num = sscanf(val, "%d", &connect);
	if (num != 1) {
		pr_info("%s: get val error\n", __func__);
		return -EINVAL;
	}

	if (connect)
		sc27xx_typec_set_rp_rd(6);
	else 
		sc27xx_typec_set_rp_rd(0);

	return 0;
}

struct zte_misc_ops cc_connect_node = {
	.node_name = "cc_connect",
	.set = cc_connect_set,
	.get = NULL,
	.free = NULL,
	.arg = NULL,
...
#ifdef ZTE_FEATURE_SOC_RESERVED
static int zte_get_soc_reserved_flag(void)
{
	...
	s = strstr(cmd_line, "zte.soc.reserved.disable=");
	if (!s) {
		pr_err("can't find zte.soc.reserved.disable\n");
		return -1;
	}

	sscanf(s, "zte.soc.reserved.disable=%s", val_buf);
	...
	if (value == 1) {
		zte_soc_reserved_flag = 0;
	}

	return 0;
}

static int zte_soc_reserved_en_set(const char *val, const void *arg)
{
	struct charger_manager *cm = (struct charger_manager *) arg;
	int zte_soc_res_en = -1;
	int num = 0;
	...
	/*(void *) arg;*/
	num = sscanf(val, "%d", &zte_soc_res_en);
	pr_info("%s: set zte_soc_res_en %d\n", __func__, zte_soc_res_en);
	if (num != 1) {
		pr_err("%s: wrong args num %d. usage: echo <0 or 1> > zte_soc_res_en\n", __func__, num);
		return -EINVAL;
	}
	...
	if (zte_soc_res_en == 0)
		...
		zte_soc_reserved_flag = 1;

	/* Update ui_soc when user changed the SOC Reserved Mode */
	queue_delayed_work(cm->cm_cap_wq,
				&cm->cap_update_work,
...
static int zte_soc_reserved_en_get(char *val, const void *arg)
{
	pr_info("%s: get zte_soc_reserved_flag: %d\n", __func__, zte_soc_reserved_flag);

	return snprintf(val, PAGE_SIZE, "%d", zte_soc_reserved_flag);
}

struct zte_misc_ops zte_soc_reserved_node = {
	.node_name = "zte_soc_reserved",
	.set = zte_soc_reserved_en_set,
	.get = zte_soc_reserved_en_get,
	.free = NULL,
	.arg = NULL,
};
#endif
...
static int charger_manager_probe(struct platform_device *pdev)
{
...
#ifdef ZTE_FEATURE_SOC_RESERVED
	zte_get_soc_reserved_flag();
	if (zte_soc_reserved_flag == 1) {
		zte_reserved_soc_conversion(ZTE_BATTERY_RESERVED_SOC, CM_CAP_FULL_PERCENT, cm->desc->cap, &zte_conv_ui_cap);
		dev_info(cm->dev, "get_boot_cap cap = %d, zte_conv_ui_cap = %d\n", cm->desc->cap, zte_conv_ui_cap);
	}
	dev_info(cm->dev, "zte_soc_reserved_flag = %d, get_boot_cap cap = %d\n", zte_soc_reserved_flag, cm->desc->cap);
	zte_misc_register_callback(&zte_soc_reserved_node, cm);
#endif
	...
	zte_misc_register_callback(&cc_connect_node, cm);
#ifdef ZTE_FEATURE_PV_AR
