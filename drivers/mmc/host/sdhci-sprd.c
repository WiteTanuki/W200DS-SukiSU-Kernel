...
static inline void sdhci_sprd_writeb(struct sdhci_host *host, u8 val, int reg)
{
	...
	if (unlikely(reg == SDHCI_SOFTWARE_RESET)) {
		 ...
		 * data transfer. So we can check SDHCI_PRESENT_STATE register on
		 * Spreadtrum's platform to avoid this situation.
		 */
...
static void sdhci_sprd_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (true == en) {
		...
		tmp &= ~ SDHCI_SPRD_DLL_ALL_CPST_EN;
		sdhci_writel(host, tmp, SDHCI_SPRD_REG_32_DLL_CFG);
		/* wait 1ms */
		usleep_range(1000, 1250);
	}
...
static int sdhci_sprd_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	 ...
	 * if a sd card failed in tuning CMD and DATA line together,
	...
	if (!strcmp(mmc_hostname(mmc), "mmc1") && err && sprd_host->tuning_merged) {
		pr_err("%s: cmd and data tuning together failed, afterwards tuning separately\n",
			mmc_hostname(mmc));
