...
static struct ufs_dev_fix ufs_fixups[] = {
	...
	UFS_FIX(UFS_VENDOR_TOSHIBA, "THGLF2G9D8KBADG",
		UFS_DEVICE_QUIRK_PA_TACTIVATE),
	UFS_FIX(UFS_VENDOR_YMTC, "YMUS8A1TC2A2C1",
		UFS_DEVICE_QUIRK_YMTC_PA_TACTIVATE),
	UFS_FIX(UFS_VENDOR_YMTC, "YMUS9A1TC3A2C1",
		UFS_DEVICE_QUIRK_YMTC_PA_TACTIVATE),
...
static void ufshcd_tune_unipro_params(struct ufs_hba *hba)
{
	...
	if (hba->dev_quirks & UFS_DEVICE_QUIRK_YMTC_PA_TACTIVATE) {
	dev_err(hba->dev, "%s: ymtc add tminactive\n", __func__);
	ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TACTIVATE), 4);
	ufshcd_dme_peer_set(hba, UIC_ARG_MIB(0x15D0), 0x4F);
	ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TACTIVATE), 9);
	ufshcd_dme_set(hba, UIC_ARG_MIB(0x15D0), 0x4F); }
}
