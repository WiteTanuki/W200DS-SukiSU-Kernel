...
static char alsps_info[10] = {0};  /* add alsps chip infor for factory cali data collected */
	...
	do {
		...
		if (nwrite < len || nwrite < 0) {
			...
			if (sensor->sensorhub_init_ready == true) {
				...
				zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_SENSORHUB_RW_ERROR_NO);
				wr_zlog_times++;
			}
#endif
...
{
#ifndef CONFIG_ZTE_SENSOR_CALI_SUPPORT
	return;
#else
	int sensor_type = SENSOR_PROXIMITY;
	struct proximity_factory_cali_data prox_cali_data;

	memcpy(&prox_cali_data, data, sizeof(struct proximity_factory_cali_data));
...
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	zlog_client_record(zlog_sprd_sensorhub_client, "prox cali data: %d %d %d\n",
				prox_cali_data.ground_noise,
				prox_cali_data.cali_ground_noise,
				prox_cali_data.real_ground_noise);
	zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_FACTORY_CALI_NO);
	ZTE_SENSOR_LOG_INFO("sensor_type = %d prox_cali_data = %d %d %d send to zlog\n",
			sensor_type,
			prox_cali_data.ground_noise,
			prox_cali_data.cali_ground_noise,
			prox_cali_data.real_ground_noise);
	...
	ZTE_SENSOR_LOG_INFO("sensor_type = %d prox_cali_data = %d %d %d\n",
			sensor_type,
			prox_cali_data.ground_noise,
			prox_cali_data.cali_ground_noise,
			prox_cali_data.real_ground_noise);
			    ...
			    const char *buf, size_t count)
{
	...
	ZTE_SENSOR_LOG_INFO("handle = %d, enabled = %d\n", handle, enabled);
			   ...
			   const char *buf, size_t count)
{
	...
	ZTE_SENSOR_LOG_INFO("handle = %d, rate = %d, batch_latency = %lld\n",
			 batch_cmd.handle,
					...
					int sensor_type, u8 *data)
{
	...
	ZTE_SENSOR_LOG_INFO("sensor_type = %d sensor->cali_data = [%d %d %d]\n",
			sensor_type, cali_data[0], cali_data[1], cali_data[2]);
...
/* add for light factory cali data collected start */
	if (sensor_type == SENSOR_LIGHT) {
		als_cali_coef = cali_data[0];
		ZTE_SENSOR_LOG_INFO("als_cali_coef set to %d\n", als_cali_coef);
	}
...
/* add for prox factory cali data collected start */
	if (sensor_type == SENSOR_PROXIMITY) {
		/* add real_ground_noise start */
		memcpy(&prox_cali_data, data, sizeof(struct proximity_factory_cali_data));
#ifdef CONFIG_ZTE_SENSOR_CALI_SUPPORT
		ZTE_SENSOR_LOG_INFO("sensor_type = %d prox_cali_data = %d %d %d\n",
				sensor_type,
				prox_cali_data.ground_noise,
				prox_cali_data.cali_ground_noise,
				prox_cali_data.real_ground_noise);
#else
		ZTE_SENSOR_LOG_INFO("sensor_type = %d prox_cali_data = %d %d\n",
				sensor_type,
				prox_cali_data.ground_noise,
				prox_cali_data.real_ground_noise);
#endif
		ps_cali_ct = (prox_cali_data.real_ground_noise > 0) ?
				prox_cali_data.real_ground_noise : prox_cali_data.ground_noise;
		/* add real_ground_noise end */
		// ps_cali_ct = cali_data[0];
		ZTE_SENSOR_LOG_INFO("ps_cali_ct set to %d\n", ps_cali_ct);

		if (prox_cali_data.real_ground_noise != prox_cali_data.ground_noise) {
			ZTE_SENSOR_LOG_INFO("prox real_ground_noise %d, ground_noise %d prox cali failed\n",
				prox_cali_data.real_ground_noise, prox_cali_data.ground_noise);
			return -1;
		}
	}
/* add for prox factory cali data collected end */
	...
	case SENSOR_PROXIMITY:
		...
		zlog_client_record(zlog_sprd_sensorhub_client, "prox cali data: %d %d %d\n",
				...
				prox_cali_data.cali_ground_noise,
				prox_cali_data.real_ground_noise);
		ZTE_SENSOR_LOG_INFO("sensor_type = %d sensor->cali_data = %d %d %d send to zlog\n",
				sensor_type,
				prox_cali_data.ground_noise,
				prox_cali_data.cali_ground_noise,
				prox_cali_data.real_ground_noise);
		...
		zlog_client_record(zlog_sprd_sensorhub_client, "prox cali data: %d %d\n",
				prox_cali_data.ground_noise,
				prox_cali_data.real_ground_noise);
		ZTE_SENSOR_LOG_INFO("sensor_type = %d sensor->cali_data = %d %d send to zlog\n",
				sensor_type,
				prox_cali_data.ground_noise,
				prox_cali_data.real_ground_noise);
#endif
	...
	zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_FACTORY_CALI_NO);
	ZTE_SENSOR_LOG_INFO("sensor_type = %d cali_data = %d %d %d send to zlog\n",
			sensor_type, cali_data[0], cali_data[1], cali_data[2]);
#endif
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (sensor->cal_cmd == CALIB_DATA_READ) {
			...
			if (err < 0) {
				ZTE_SENSOR_LOG_ERROR("Save Sensor CalibratorData Fail\n");
				return err;
			}
			return sprintf(buf, "%d\n", err);
	}
				     ...
				     const char *buf, size_t count)
{
	...
	ZTE_SENSOR_LOG_INFO("sensor_type = %d cali_data = %d %d %d send to sensorhub\n",
			sensor_type, cali_data[0], cali_data[1], cali_data[2]);

/* add for light factory cali data collected start */
	if (sensor_type == SENSOR_LIGHT) {
		als_cali_coef = cali_data[0];
		ZTE_SENSOR_LOG_INFO("als_cali_coef init to %d\n", als_cali_coef);
	}
/* add for light factory cali data collected end */
...
/* add for prox factory cali data collected start */
	if (sensor_type == SENSOR_PROXIMITY) {
		...
		memcpy(&prox_cali_data, data, sizeof(struct proximity_factory_cali_data));
#ifdef CONFIG_ZTE_SENSOR_CALI_SUPPORT
		ZTE_SENSOR_LOG_INFO("prox_cali_data ground_noise:%d cali_ground_noise:%d real_ground_noise:%d\n",
				prox_cali_data.ground_noise,
				prox_cali_data.cali_ground_noise,
				prox_cali_data.real_ground_noise);
#else
		ZTE_SENSOR_LOG_INFO("prox_cali_data ground_noise:%d real_ground_noise:%d\n",
				prox_cali_data.ground_noise,
				prox_cali_data.real_ground_noise);
#endif
		ps_cali_ct = (prox_cali_data.real_ground_noise > 0) ?
		...
		/* add real_ground_noise end*/
		// ps_cali_ct = cali_data[0];
		ZTE_SENSOR_LOG_INFO("ps_cali_ct init to %d\n", ps_cali_ct);
	}
/* add for prox factory cali data collected end */
			    ...
			    char *buf)
{
	...
	if (err >= 0) {
		...
		ZTE_SENSOR_LOG_INFO("CM4 Version:%u\n", version);
		...
		ZTE_SENSOR_LOG_ERROR("Read  FW Version Fail\n");
		return err;
	}
...
/* add alsps chip infor for factory cali data collected start */
static ssize_t alsps_info_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return snprintf(buf, sizeof(alsps_info), "%s\n", alsps_info);
}
static DEVICE_ATTR_RO(alsps_info);
/* add alsps chip infor for factory cali data collected end */
				...
				struct device_attribute *attr, char *buf)
{
	...
	ZTE_SENSOR_LOG_INFO("sensor_infor_send_over = %d",
			sensor->sensor_infor_send_over);
	...
	for (i = 0; i < ARRAY_SIZE(sensor->sensor_info_list); i++) {
		if (sensor->sensor_info_list[i].name[0] != 0) {
			...
			/* add alsps chip infor for factory cali data collected start */
			if ((strncmp(sensor->sensor_info_list[i].name, "prox_", 5)) == 0)
				strcpy(alsps_info, sensor->sensor_info_list[i].name + 5);
			/* add alsps chip infor for factory cali data collected end */
		}
	}
	return len;
				 ...
				 const char *buf, size_t count)
{
	...
	if (ret > 0)
		ZTE_SENSOR_LOG_INFO("set_brightness = %d\n", sensor->brightness);
				 ...
				 const char *buf, size_t count)
{
	...
	if (ret > 0)
		ZTE_SENSOR_LOG_INFO("set_actual_fps = %d\n", sensor->actual_fps);
...
static ssize_t sensor_selftest_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	u8 buffer[6];
	int ret, sensor_handle;
	struct shub_data *sensor = dev_get_drvdata(dev);

	memcpy(buffer, buf, sizeof(buffer));

	ret = sscanf(buffer, "%d", &sensor_handle);
	if (ret > 0)
		ZTE_SENSOR_LOG_INFO("sensor_selftest sensor_handle = %d\n", sensor_handle);

	sensor->sensor_selftest_over = false;
	sensor->sensor_selftest_res = 0;

	shub_send_command(sensor, sensor_handle,
			  SHUB_SENSOR_SELF_TEST_SUBTYPE, buffer,
			  sizeof(buffer));

	return count;
}

static ssize_t sensor_selftest_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct shub_data *sensor = dev_get_drvdata(dev);
	int ret;
	...
	wait_event_interruptible_timeout(sensor->sensor_selftest_wq,
						(sensor->sensor_selftest_over),
						msecs_to_jiffies
						(RECEIVE_SENSORHUB_EVENT_TIMEOUT_MS));

	ZTE_SENSOR_LOG_INFO("sensor_selftest_over = %d",
			sensor->sensor_infor_send_over);

	if (sensor->sensor_selftest_res == 1)
		ret = 0;
	else
		ret = -1;
...
static DEVICE_ATTR_RW(sensor_selftest);
...
static struct attribute *sensorhub_attrs[] = {
	...
	&dev_attr_alsps_info.attr,  /* add alsps chip infor for factory cali data collected */
...
static int shub_algo_cmd_notifier_fn(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	if (sensor->sensorhub_init_ready == false) {
		ZTE_SENSOR_LOG_INFO("sensorhub_init_ready false, return.\n");
		return NOTIFY_OK;
	}
	...
	ZTE_SENSOR_LOG_INFO("SHUB_ALGO_CMD_SET_BRIGHTNESS brightness = %d\n",
			sensor->brightness);
	shub_send_command(sensor, SENSOR_LIGHT,
			  SHUB_SET_BRIGHTNESS_SUBTYPE, buffer,
			  sizeof(buffer));
	...
	ZTE_SENSOR_LOG_INFO("SHUB_ALGO_CMD_SET_ACTUAL_FPS actual_fps = %d\n",
			sensor->actual_fps);
	shub_send_command(sensor, SENSOR_PROXIMITY,
			  SHUB_SET_ACTUAL_FPS_SUBTYPE, buffer,
			  sizeof(buffer));
		break;
	default:
		ZTE_SENSOR_LOG_INFO("wrong action %d\n", action);
		break;
	}
...
/* add under-panel light sensor algo end */
	...
	init_waitqueue_head(&mcu->sensor_selftest_wq);
	mcu->sensor_selftest_over = false;

#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	zlog_sprd_sensorhub_client = zlog_register_client(&zlog_sprd_sensorhub_dev);
	if (!zlog_sprd_sensorhub_client)
		ZTE_SENSOR_LOG_ERROR("zlog register client zlog_sprd_sensorhub_dev failed\n");
#endif
