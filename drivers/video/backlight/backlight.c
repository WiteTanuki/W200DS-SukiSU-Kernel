...
#ifdef ZTE_FEATURE_CONTROL_BACKLIGHT_BY_VOLTAGE
static unsigned long custom_brightness = 255;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rc)
		return rc;
	if (bd->ops) {
		pr_info("zte v2 set custom_brightness to %lu\n", brightness);
		if (brightness < custom_brightness) {/* down */
			custom_brightness = brightness;
			if (brightness < curr_brightness)
				rc = backlight_device_set_brightness(bd, brightness);
		} else { /* up */
			if (custom_brightness == curr_brightness) {
				custom_brightness = brightness;
				rc = backlight_device_set_brightness(bd, brightness);
			} else {
				custom_brightness = brightness;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	#ifdef ZTE_FEATURE_CONTROL_BACKLIGHT_BY_VOLTAGE
	/*brightness = (brightness >= custom_brightness) ? custom_brightness : brightness;*/
	if (brightness > custom_brightness) {
		pr_info("zte will set brightness from %lu to %lu\n", brightness, custom_brightness);
		brightness = custom_brightness;
	}
	#endif
