...
static void dwc3_core_exit(struct dwc3 *dwc)
{
	...
	dev_info(dwc->dev, "%s flag:0x%x\n", __func__, dwc->usb3_phy->flags);
	mutex_lock(&dwc->mutex);
	if (dwc->usb3_phy->flags & USB_INIT_FLAG) {
			dwc->usb3_phy->flags &= ~USB_INIT_FLAG;
...
static int dwc3_suspend(struct device *dev)
{
	...
	dev_info(dev, "dwc3 pm suspend enter\n");
...
static int dwc3_resume(struct device *dev)
{
	...
	dev_info(dev, "dwc3 pm resume enter\n");
