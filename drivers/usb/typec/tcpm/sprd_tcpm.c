...
static int tcpm_entermode_cnt;
static int tcpm_attention_cnt;
...
static void sprd_tcpm_register_partner_altmodes(struct sprd_tcpm_port *port)
{
	...
	for (i = 0; i < modep->altmodes; i++) {
		if (IS_ERR_OR_NULL(port->partner_altmode[i])) {
			altmode = typec_partner_register_altmode(port->partner, &modep->altmode_desc[i]);
			sprd_tcpm_log(port, "register partner(%d) SVID:0x%04x, p:%p", i, modep->altmode_desc[i].svid, altmode);
			if (IS_ERR(altmode)) {
			...
			sprd_tcpm_log(port, "altmode is already registered, SVID 0x%04x",
						modep->altmode_desc[i].svid);
		}
	}
}
			     ...
			     const __le32 *payload, int cnt, u32 *response)
{
	...
	case SPRD_CMDT_INIT:
		...
		case SPRD_CMD_ATTENTION:
			tcpm_attention_cnt++;
			sprd_tcpm_log(port, "%s ! LINE%d! tcpm_attention_cnt is %d\n",
				__func__, __LINE__, tcpm_attention_cnt);
					 ...
					 const __le32 *payload, int cnt)
{
	...
	* Currently not supported: Unstructured VDM message type.
	* Reseason: Unstructured VDM is a customized pd private aggreement.
	*/
			       ...
			       u32 vid, int cmd, const u32 *data, int count)
{
	...
	sprd_tcpm_log(port, "call vdm_state_machine timeout:%d", timeout);
	/*mod_delayed_work(port->wq, &port->vdm_state_machine, 0);*/
	mod_delayed_work(port->wq, &port->vdm_state_machine, msecs_to_jiffies(timeout));
}
...
static int sprd_tcpm_altmode_enter(struct typec_altmode *altmode)
{
	...
	tcpm_entermode_cnt++;
	sprd_tcpm_log(port, "%s ! LINE%d! tcpm_entermode_cnt is %d\n",
				__func__, __LINE__, tcpm_entermode_cnt);
	mutex_lock(&port->lock);
...
static void sprd_tcpm_unregister_altmodes(struct sprd_tcpm_port *port)
{
	...
	for (i = 0; i < modep->altmodes; i++) {
		sprd_tcpm_log(port, "unregister partner(%d) SVID:0x%04x, p:%p", i, modep->altmode_desc[i].svid, port->partner_altmode[i]);
				 ...
				 enum sprd_typec_cc_status cc2)
{
	...
	sprd_tcpm_log(port, "%s ! LINE%d! tcpm_entermode_cnt is %d, tcpm_attention_cnt is %d\n",
				__func__, __LINE__, tcpm_entermode_cnt, tcpm_attention_cnt);
...
void sprd_tcpm_unregister_port(struct sprd_tcpm_port *port)
{
	...
	for (i = 0; i < ARRAY_SIZE(port->port_altmode); i++) {
		sprd_tcpm_log(port, "unregister partner(%d) p:%p", i, port->partner_altmode[i]);
