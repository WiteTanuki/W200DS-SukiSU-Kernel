...
static int sc27xx_pd_set_vbus(struct tcpc_dev *tcpc, bool on, bool charge)
{
	...
	if (pd->vbus_on == on) {
		...
		if (on) {
			...
			dev_err(pd->dev, "sc27xx_pd call chargeIC open otg-vbus\n");
			...
			dev_err(pd->dev, "sc27xx_pd call chargeIC close otg-vbus\n");
...
static int sc27xx_pd_check_vbus_cc_status(struct sc27xx_pd *pd)
{
	...
	if (pd->state == SC27XX_ATTACHED_SRC) {
		...
		sprd_pd_log(pd, "mason sc27xx_pd_set_typec_rp_level is SPRD_TYPEC_CC_RP_3_0");
	}
...
static void sc27xx_pd_extcon_dp(struct sc27xx_pd *pd)
{
#if IS_ENABLED(CONFIG_SPRD_TYPEC_DP_ALTMODE)
	union extcon_property_value hpd_status;
