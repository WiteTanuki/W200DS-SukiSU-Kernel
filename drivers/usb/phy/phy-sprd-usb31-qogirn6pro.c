...
struct sprd_ssphy {
	...
	bool                    refout_is_off;
	struct delayed_work	usb_retry_detect_work;
};
...
static int sprd_ssphy_set_vbus(struct usb_phy *x, int on)
{
	...
	if (x->last_event == USB_EVENT_ID && on == false) {
		dev_info(x->dev, "don't set devcie mode when ID ground!\n");
		return ret;
	}

	dev_info(x->dev, "%s enter, %d\n", __func__, on);
	if (on) {
...
static int sprd_ssphy_set_wakeup(struct usb_phy *x, bool keep_wakeup)
{
	struct sprd_ssphy *phy = container_of(x, struct sprd_ssphy, phy);
	u32 reg = 0;
...
static int sprd_ssphy_init(struct usb_phy *x)
{
	...
	dev_info(x->dev, "%s phy->phy.flags:0x%x .\n", __func__, phy->phy.flags);
	//need usb and dp all not plugin before
	if ((phy->phy.flags & USB_INIT_FLAG) || phy->phy.flags & DP_INIT_FLAG) {
		dev_info(x->dev, "%s  USB has inited.\n", __func__);
		return 0;
	}
	...
	if (!sprd_usbm_hsphy_get_onoff()) {
		...
		if (phy->is_host) {
                  pr_err("%s:phy is host\n", __func__);
			regmap_write(phy->ana_g0l, REG_ANLG_PHY_G0L_ANALOG_USB20_USB20_TRIMMING,
				phy->host_eye_pattern);
		} else {
                  pr_err("%s:phy is device\n", __func__);
			regmap_write(phy->ana_g0l, REG_ANLG_PHY_G0L_ANALOG_USB20_USB20_TRIMMING,
				phy->device_eye_pattern);
		}
	}
...
static void sprd_ssphy_shutdown(struct usb_phy *x)
{
	...
	//need usb and dp all  plugout
	if (((phy->phy.flags & USB_INIT_FLAG) || (phy->phy.flags & DP_INIT_FLAG))) {
		dev_info(x->dev, "%s  combophy has used.\n", __func__);
		return;
	}
				...
				unsigned long event, void *data)
{
	...
	dev_info(usb_phy->dev, "sprd: %s [%d], event=%d\n",
	...
	if (event) {
		sprd_usb_changed(&phy->bc1p2_info, USB_CHARGER_PRESENT);
		schedule_delayed_work(&phy->usb_retry_detect_work, msecs_to_jiffies(8000));
	} else {
		cancel_delayed_work(&phy->usb_retry_detect_work);
		sprd_usb_changed(&phy->bc1p2_info, USB_CHARGER_ABSENT);
	}
#endif
...
static void usb_retry_charger_detect_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct sprd_ssphy *phy = container_of(dwork, struct sprd_ssphy,
								usb_retry_detect_work);

	pr_info("%s chg_type=%d\n", __func__, phy->phy.chg_type);

	if (phy->phy.chg_type == SDP_TYPE && !get_sdp_auth_status()) {
		pr_info("%s retry trigger\n", __func__);
		phy->phy.chg_type = sprd_bc1p2_retry_detect(&phy->phy);
		usb_phy_notify_charger(&phy->phy);
	}
}
...
static int sprd_ssphy_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&phy->usb_retry_detect_work, usb_retry_charger_detect_work);
	...
	phy->phy.set_wakeup			= sprd_ssphy_set_wakeup;
	/*
