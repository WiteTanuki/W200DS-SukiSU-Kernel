...
{
    if (unlikely(!susfs_zygote_sid)) {
        susfs_zygote_sid = susfs_get_sid_from_name("u:r:zygote:s0");
        if (!susfs_zygote_sid) {
            pr_warn("zygote SID is unavailable during setresuid\n");
            return 0;
        }
        pr_info("recovered zygote SID during setresuid: %u\n", susfs_zygote_sid);
    }
    ...
    if (unlikely(ruid == WEBVIEW_ZYGOTE_UID) && ksu_uid_should_umount(ruid))
        goto do_umount;
