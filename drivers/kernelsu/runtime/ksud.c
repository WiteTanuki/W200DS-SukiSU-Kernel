...
void on_post_fs_data(void)
{
    ...
    ksu_load_allow_list();
    ksu_observer_init();
    /* Existing managers may not cause a packages.list event after this point. */
    track_throne(false);
	/* Safe mode only needs early-boot input samples. */
	stop_input_hook();
...
void on_boot_completed(void)
{
    ...
    /* Retry discovery as well as pruning in case post-fs-data ran too early. */
    track_throne(false);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
                             ...
                             struct user_arg_ptr *envp, int *flags)
{
    ...
    if (current->pid != 1 && is_init(get_current_cred())) {
        if (unlikely(strcmp(filename->name, KSUD_PATH) == 0)) {
			...
			if (ret)
				pr_err("ksud: init credential setup failed: %d\n", ret);
        }
    }
...
{
	/* Do not need to check again; userspace may call multiple times. */
	static bool already_checked;

	if (already_checked)
		return true;

	/* Stop sampling before publishing the result to userspace. */
	stop_input_hook();

	if (!READ_ONCE(safe_mode_flag))
		return false;
...
static void vol_detector_event(struct input_handle *handle, unsigned int type, unsigned int code, int value)
{
	...
	if (vol_up_cnt >= VOLUME_PRESS_THRESHOLD_COUNT ||
		...
		pr_info("volume keys pressed max times, safe mode detected!\n");
		WRITE_ONCE(safe_mode_flag, true);
	}
}
...
static int vol_detector_init(void)
{
	...
	if (ret) {
		...
		pr_err("vol_detector: failed to register: %d\n", ret);
	}

	return ret;
}

static void vol_detector_exit(void)
{
