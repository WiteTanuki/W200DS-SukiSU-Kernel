...
static int do_report_event(void __user *arg)
{
    ...
    case EVENT_BOOT_COMPLETED: {
        ...
        if (!boot_complete_lock) {
            ...
            on_boot_completed();
#if defined(CONFIG_KSU_SUSFS) && defined(SUSFS_HAS_SDCARD_MONITOR)
...
int ksu_handle_sys_reboot(int magic1, int magic2, unsigned int cmd, void __user **arg)
{
    ...
    if (magic2 == SUSFS_MAGIC && current_uid().val == 0) {
		...
		ret = susfs_add_open_redirect(*arg);
...
static int do_set_spoof_version(void __user *arg)
{
    ...
    return ksu_set_spoof_version(cmd.release[0] ? cmd.release : NULL,
                                 cmd.version[0] ? cmd.version : NULL);
}
...
#ifdef CONFIG_KPM
	cmd.enabled = sukisu_kpm_get_state() == SUKISU_KPM_RUNTIME_READY;
#endif
